...
7개 (10개 미만 권장선 안쪽). AI 클라이언트가 **두 층의 기억(의미층 + 서사층)을 항해**하도록 설계. recall_memory와 explore_topic으로 빠르게 요약을 받고, search_passages / get_episode_excerpt로 근거까지 깊게 드릴다운한다.

**역할별 묶음**:
- **기본 조작** (3): manage_workspace, initialize_memory, store_memory
- **의미층 검색** (2): recall_memory (앵커 매칭 + 구조화 fact), explore_topic (지형도)
- **서사층 검색** (2): search_passages (풀 전역 의미 검색), get_episode_excerpt (단일 에피소드 깊이 조회)

재구성 패턴: `recall_memory → fact + episode_id → get_episode_excerpt` 2-stage (Graphiti의 `search_facts + get_episodes` 등가물). 이 분리로 tool 스키마 토큰 비용을 최소화하면서 "왜/어떻게/비교" 같은 서사 질문에도 답변 가능.
