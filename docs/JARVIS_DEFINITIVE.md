...
> 최종 갱신: 2026-04-01 (경쟁 환경 + 대화 분할 + 시각화 UX 반영)
> 상태: 초안 — 빈 곳을 채워나가며 확정
...
결과: 서버에 LLM 추론 비용이 0이다. 로컬 임베딩 모델(~500MB)과 문자열 매칭만 사용하며, 외부 API 의존 없이 서버 하나에서 자체 완결된다.
...
│    └── Episode (대화 원문 보존, 불변, 트리플 트리거로 증분 저장)
├── Entity (엔티티 노드: person, project, technology, file, concept, ...)
├── KnowledgeFact (구조화된 사실, bitemporal)
...
## 7. 저장 파이프라인 (store_memory)

AI 클라이언트가 대화 중 자동 호출. 호출 시점은 **트리플 트리거**:

1. **토픽 전환**: 대화 주제가 바뀌었을 때 (AI가 판단)
2. **고정 간격 폴백**: 5턴 이상 store 없이 지나갔을 때 (안전망)
3. **중요 이벤트**: 결정, 선호도 변경, 사실 정정 등이 발생했을 때

리서치 결과: "every 3-5 turns" 프레이밍보다 **"when you learn something new"** 프레이밍이 기억 품질이 더 높다. 고정 간격은 데이터 유실 방지용 안전망으로만 사용.
...
6. KnowledgeFact 생성
   → 같은 entity + predicate에 기존 사실이 있으면 supersede 처리
   → (기존 사실 superseded_at = NOW, 새 사실 insert — 원자적)

7. EntityRelation 생성

→ 1~7이 하나의 DB 트랜잭션. 하나라도 실패하면 전체 롤백.
...
3개. 도구 수를 최소로 유지해야 AI 클라이언트의 컨텍스트 비용이 관리 가능하다 (10개 미만 권장, 30개 넘으면 정확도 급락).

### initialize_memory

- **언제 호출**: 세션 시작 시 (이름이 "initialize"라서 AI가 높은 확률로 자동 호출)
- **뭘 보냄**: workspace_id
- **서버가 반환**: 워크스페이스 최근 맥락 요약 + 메모리 프로토콜 지침:
  - "새로운 사실을 배울 때마다 store_memory 호출"
  - "5턴 이상 저장 없으면 반드시 평가"
  - "각 기억은 원자적 — 하나의 사실, 하나의 self-contained 문장"
- **존재 이유**: MCP `instructions` 필드를 무시하는 클라이언트가 많음. 이 도구가 사실상 instructions의 역할을 대체

### store_memory

- **언제 호출**: 트리플 트리거 (토픽 전환 / 5턴 폴백 / 중요 이벤트)
- **뭘 보냄**: 대화 원문 + 엔티티/사실/관계 힌트 + provider
- **서버가 하는 일**: 검증 → 저장 → 비동기 임베딩
...
- MCP instructions에 "MUST call" 명시
- 세션당 store 호출 0회 감지 → 경고
- 모니터링: 세션별 tool invocation rate 추적

### 긴 대화에서 앞부분 유실

- 트리플 트리거로 증분 저장 (토픽 전환 / 5턴 폴백 / 중요 이벤트 — 대화 끝에 한 번이 아님)
- 이것이 데이터 손실 방지의 핵심 설계
...
- Bitemporal supersede: single-valued predicate는 latest-wins
- Multi-valued predicate는 공존 허용
