...
├── Fragment (자연어 텍스트 조각, 500자 이내) — 시맨틱 검색용
│    └── content = source_quote (원문 대화 맥락, ≥10자면 사용, 아니면 triple fallback)
│    └── type(fact/decision/error/preference/procedure/relation)
│    └── keywords[] (content에서 추출한 명사 토큰, 20개 cap)
│    └── importance (0.7 grounded / 0.4 low_trust)
│    └── source_episode_id, source_fact_id
│    └── 새 기억마다 KnowledgeFact + Fragment 양쪽 저장
│    └── Fragment embedding으로 벡터 검색 (KnowledgeFact triple이 아님)
│    └── 핵심 원칙: 응답 데이터에 이미 structured triple 포함 → Fragment는 응답에 없는 축(원문 맥락)을 검색에 담는 역할
...
5. 엔티티 해소 (3단계 파이프라인)
   → Stage 1: 정규화 + 별칭 사전 (<1ms)
     ...
     · 기술 별칭 사전 (ALIAS_DICT): "포스트그레스"→"postgresql", "k8s"→"kubernetes" 등
     · 크로스링구얼 별칭 사전 (CROSS_LINGUAL_ALIASES): "자비스"→"JARVIS", "세컨드브레인"→"SecondBrain" 등
       - 임베딩 cosine만으로 한국어↔영어 음차 매칭이 약함 (실측 0.4 수준, threshold 0.75 미달)
       - 실측된 프로젝트/도메인 쌍만 수동 등록, 투기적 확장 금지
       - 운영 중 발견되는 새 쌍을 배치로 추가
...
### 서버 처리 순서 — 2단계 구조: hybrid search → 맥락 조립

**Stage 0: 쿼리 전처리** (<1ms)

저장 측의 정규화와 대칭 적용. 생략 시 FTS가 자연어 쿼리에서 거의 매칭 실패 (실측: 다중 단어 쿼리는 PGroonga `&@~` AND 기본 동작으로 0건).

- Unicode NFKC 정규화, 소문자화 → 임베딩 입력용
- 한국어 파티클 제거 + 영어 불용어 제거 → 토큰 추출
- 별칭 사전 확장 (CROSS_LINGUAL_ALIASES + ALIAS_DICT) → 원본과 alias 양쪽 유지
- PGroonga OR 쿼리 조립 ("자비스 구현" → `JARVIS OR 자비스 OR 구현`)

**Stage 1: 하이브리드 검색 (pool=100 고정)**

3가지 검색을 RRF로 합산. 사용자 `limit`과 무관한 고정 pool 크기로 Stage 2 MMR이 다양화할 여지 확보.

**① 벡터 검색 (Fragment 임베딩 기반)**
- 질의를 multilingual-e5-small-ko로 임베딩 (query: 접두사)
- Fragment의 content(= source_quote, 자연어) 임베딩과 cosine similarity
- 주의: KnowledgeFact triple이 아닌 Fragment content를 인덱싱 — 응답 데이터에 이미 structured triple 포함, 중복 인덱싱 의미 없음. Fragment는 응답에 없는 축(원문 대화 맥락)을 검색에 담는 역할

**② 전문 검색 (PGroonga)**
- 한국어+영어 혼합 자동 처리 (CJK는 N-gram, Latin은 word 토큰화)
- 검색 대상: `entity.name`, `knowledge_facts.object_value`, `knowledge_facts.source_quote`
- `&@~` 연산자는 pgroonga 쿼리 문법 — 다중 단어는 AND 기본. Stage 0에서 OR 명시 필수
...
**Stage 2: 맥락 조립 (community-aware MMR + adaptive K)** (<5ms)

Stage 1의 100개 후보에서 "관련성 × 다양성" 최적화된 부분집합 선택. flat ranked list의 편향(grab-bag fact 상위 점유, 한 카테고리 독점)을 해결.

- **Soft decay 적용 (sim_1 계산 전)** → `final_score = rrf_score × importance × e^(-λ × days_since_access)`
  - 유형별 반감기: preference=120일, decision=90일, fact=60일, procedure=30일, error=60일, relation=60일
  - importance는 Fragment.importance를 JOIN해서 가져옴 (KnowledgeFact에 직접 컬럼 없음)
  - last_accessed_at은 recall 시 선택된 fact들에 NOW() 갱신 → "다시 접근하면 importance 복원"
- **MMR 재정렬** (λ=0.6, community bonus +0.05)
  - sim_1: min-max 정규화된 final_score
  - sim_2: fact 임베딩 cosine 유사도 (중복 페널티)
  - community_bonus: 선택된 커뮤니티 집합에 없는 community_id면 가산
- **Adaptive K**: marginal gain ratio τ=0.1로 자동 종료. K_min=3, K_max=20
- **응답 포맷 확장**:
  - `results`: 선택된 fact 리스트 (기존 호환)
  - `coverage`: {selected_count, total_candidates, communities_represented, workspace_communities}
  - `structural_summary`: "Selected N facts from M candidates. Coverage: A of B communities. Primary entities: X, Y, Z."
  - `pagination_token`: `"more_available"` 또는 `null` (has_more 시그널)

**Community detection (오프라인)**:
- Leiden 알고리즘 (python-igraph + leidenalg)
- 엔티티 노드, EntityRelation 엣지로 그래프 구성
- 워커 배치 완료 훅에서 REINDEX와 같은 타이밍 실행 (`entities.community_id` 갱신)
- 파편화 심한 워크스페이스는 대부분 singleton 커뮤니티 → MMR의 community_bonus가 embedding fallback으로 동작 (손해 없음)

**superseded 필터링**: Stage 1 CTE에서 `kf.superseded_at IS NULL` 조건으로 현재 진실만 통과 (graph_facts, fts_facts 양쪽)
...
```json
{
  "results": [
    {
      ...
      "related_entities": [
        {"entity_id": "...", "name": "pgvector", "relation_type": "related_to", "fact_count": 3},
        {"entity_id": "...", "name": "Oracle Cloud", "relation_type": "deployed_on", "fact_count": 1}
      ],
      "history": [
  ...
  "coverage": {"selected_count": 5, "total_candidates": 100, "communities_represented": 3, "workspace_communities": 516},
  "structural_summary": "Selected 5 facts from 100 candidates. Coverage: 3 of 516 communities represented. Primary entities: JARVIS, PostgreSQL, pgvector.",
  "pagination_token": null,
  "anchor_matched": true
}
```

**2-stage 회수 흐름**: AI 클라이언트는 위 응답에서 판단 근거가 부족하면 `evidence.episode_id`를 `get_episode_excerpt(episode_id, query, max_chars=2000)`에 넘겨 원본 대화의 관련 구절(2~5KB)을 추가 회수한다. `related_entities`는 다음 recall 쿼리의 힌트로 활용 — `fact_count`가 높은 엔티티로 파고들거나, `relation_type`으로 관계 따라 이동.

---

## 9. MCP 도구

8개 (10개 미만 권장선 안쪽). AI 클라이언트가 **"항해 가능한 외부 뇌"의 4축**을 통해 자유롭게 탐색하도록 설계.

**4축 네비게이션**:
- **탐색 (broad)**: `explore_topic` — 낯선 주제의 지형도 (엔티티·커뮤니티·predicate 분포)
- **조회 (specific)**: `recall_memory` — 앵커 매칭 + 구조화 fact
- **이동 (follow)**: `follow_relation` — 엔티티 1-hop 이웃을 relation_type별로 걸어감
- **심층 (deep)**: `search_passages` (풀 전역) + `get_episode_excerpt` (단일 에피소드 내부)

**역할별 묶음**:
- **기본 조작** (3): manage_workspace, initialize_memory, store_memory
- **의미층 검색** (3): recall_memory, explore_topic, follow_relation
- **서사층 검색** (2): search_passages, get_episode_excerpt

재구성 패턴: `recall_memory → fact + episode_id → get_episode_excerpt` 2-stage (Graphiti의 `search_facts + get_episodes` 등가물). 또는 `recall_memory → related_entities → follow_relation → recall_memory` 그래프 워크. tool 스키마 분리로 토큰 비용 최소화하면서 "왜/어떻게/비교/이동" 모든 패턴 지원.
...
- **언제 호출**: 특정 엔티티·주제에 관한 구조화 사실이 필요할 때
- **뭘 보냄**: 자연어 질의 + 필터(선택)
- **서버가 반환**: 관련 사실 + 근거 excerpt + 변경 이력 + `related_entities`(relation_type, fact_count) + `anchor_matched` + `coverage`
- **2-stage 힌트**: 응답의 `evidence.episode_id`를 `get_episode_excerpt`에 넘기면 판단 근거/서사를 깊게 회수
- **응답 크기 제한**: 25,000 토큰 이내 서버 측 truncation

### explore_topic

- **언제 호출**: 낯선 주제의 지형을 먼저 훑고 싶을 때 (recall_memory 전에 정찰)
- **뭘 보냄**: workspace + 자연어 topic 쿼리
- **서버가 반환**: top 엔티티 + 커뮤니티 태그 + predicate 분포 + 시간 범위 (fact 본문 없음)
- **존재 이유**: Stage 1 pool을 그대로 통계화해 재료 비용을 22~27% 절감 (실측). fact 본문을 부르지 않으므로 "이 주제에 어떤 엔티티/predicate가 있나" 정찰용으로 저렴

### search_passages

- **언제 호출**: recall_memory가 앵커 범위 밖의 passage를 놓쳤다고 의심될 때
- **뭘 보냄**: workspace + 자연어 쿼리 + limit
- **서버가 반환**: Fragment content + similarity + episode_id + (링크된) entity·predicate
- **존재 이유**: 앵커 하드 필터를 **우회**하는 순수 fragment 의미 검색. Phase 1의 2-hop 이웃 밖에 있는 의미상 가까운 passage를 회수. 링크된 entity/predicate가 있으면 recall_memory로 follow-up 가능

### get_episode_excerpt

- **언제 호출**: 특정 에피소드 원본의 긴 맥락(2~5KB)이 필요할 때 (주로 recall_memory 후속 호출)
- **뭘 보냄**: workspace + episode_id + 쿼리 + max_chars + mode ("relevant" / "full" / "head")
- **서버가 반환**: 해당 에피소드에서 쿼리 관련 구절 + matched_keywords + total_length + passage_count
- **동작**: 에피소드 content를 요청 시점에 문단 분리, 키워드 스코어링, 원본 순서 보존하며 상위 문단 concat. 무매칭 시 `relevant_fallback_head`로 head 반환
- **존재 이유**: "왜 결정했는지", "비교 결과 뭐였는지", "어떻게 바뀌었는지" 같은 서사는 에피소드 원본에만 존재. 이 도구가 없으면 AI가 판단 근거를 재구성 불가

### follow_relation

- **언제 호출**: recall_memory가 앵커 엔티티를 확정한 뒤 그 이웃을 탐색하고 싶을 때
- **뭘 보냄**: workspace + entity(이름 또는 UUID 또는 alias) + direction("out"/"in"/"both") + relation_type 필터(선택) + limit
- **서버가 반환**: 이웃 엔티티 리스트 (각각 entity_id/name/type + relation_type + direction + fact_count + top 3 최근 fact brief) + relation_type_counts
- **존재 이유**: `related_entities`가 힌트만 주는 반면 follow_relation은 "이 관계로 어디로 갈 수 있고, 거기에 뭐가 있나"를 구체적으로 반환. AI가 그래프를 걸어다니면서 관련 정보를 수집. hub 엔티티(예: `user` 98 fact)도 자연스럽게 식별됨
- **Alias 해소**: entities.name → entity_aliases.alias 순으로 fallback (한국어 alias 포함)
...
### 해소됨 (2026-04-16~18 리서치 + 구현)

- [x] **assistant 턴 추출 필터** — Graphiti speaker-symmetric 모델 채택 (Mem0 user-only 기각). pair-level 추출 + 5-layer 기계적 필터 (block/sentence/signal/pair/decision). LLM-free. 실측 콘텐츠 5.64x, 추출 결과 10.5x
- [x] **맥락 조립 (recall Stage 2)** — community-aware MMR + adaptive K. 80% solution (Leiden 오프라인 + Python 재정렬). λ=0.6, τ=0.1, K_min=3/max=20, community_bonus=0.05. response에 coverage metadata + structural_summary 추가
- [x] **Fragment 제대로 구현** — content = source_quote (자연어, ≥10자). Fragment 임베딩 생성 + 기존 데이터 백필. vector_facts CTE가 fragment 기반으로 전환
- [x] **Soft decay 구현** — last_accessed_at 컬럼 + 반감기 사전. final_score = rrf × importance × 0.5^(days/half_life). recall 시 선택된 fact만 갱신
- [x] **쿼리 전처리** — query_preprocessing.py. NFKC + 한국어 파티클 제거 + 영어 불용어 + 별칭 확장 + PGroonga OR 조립. FTS 자연어 쿼리 0건 매칭 문제 해결
- [x] **크로스링구얼 별칭 사전** — CROSS_LINGUAL_ALIASES (자비스↔JARVIS, 세컨드브레인↔SecondBrain, 아르고스↔Argos, 펀드메신저↔fundmessenger). 임베딩 cosine 0.41로 entity resolution threshold 0.75 미달 대응
- [x] **Orphan embedding 정리** — worker 배치 훅에서 자동 cleanup. source_type별 dangling 참조 DELETE

### 미해소 / 차기 검토

- **지형도(topic map) 도구** — hierarchical retrieval 패턴. 쿼리 → 엔티티/커뮤니티/predicate/시간범위 메타만 반환 (fact 세부 없음). AI가 2단계 탐색으로 토큰 절감. 플랜 작성 완료, 구현 미착수
- **Stop 훅 + 증분 저장** — 자동 캡처. "모든 대화를 DB에 축적" 비전 달성에 필수. 설계 완료, 구현 미착수
- **OAuth 활성화 + 배포** — "어디서든 어떤 AI든 접근" 달성. 로컬 미배포 상태
