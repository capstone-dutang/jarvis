"""fix fts_facts CTE to include entity name in hybrid_graph_search

Revision ID: c3d4e5f6a7b8
Revises: b2c3d4e5f6a7
Create Date: 2026-04-16

Bug H2: FTS search missed entity names — "펀드메신저" query returned 0 results
even though it existed as an entity name. Added entities JOIN + e.name &@~ condition.
"""
...
def upgrade() -> None:
    """Recreate hybrid_graph_search with entity name in fts_facts CTE."""
    op.execute("""
...
def downgrade() -> None:
    """Restore hybrid_graph_search without entity name in fts_facts."""
    op.execute("""
        CREATE OR REPLACE FUNCTION hybrid_graph_search(
            p_workspace_id UUID,
            p_query_text TEXT,
